...
fn main() -> Result<()> {
    let matches = Command::new("toml-extract")
        ...
        .arg(
            Arg::new("field")
                ...
                .required_unless_present_any(["multiple", "package-name", "package-version", "dependencies", "authors", "keywords", "categories", "array", "array-length", "array-element"]),
        )
        .arg(
            Arg::new("multiple")
                .short('m')
    ...
    if let Some(field_paths) = matches.get_many::<String>("multiple") {
        ...
        // Single field extraction (现在支持数组语法)
